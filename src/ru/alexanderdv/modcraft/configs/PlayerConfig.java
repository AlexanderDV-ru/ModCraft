...
public class PlayerConfig extends SConfig {
	...
	public void configPlayer(IWorld world) {
		...
		for (int i = 0; i < controller.position.coords.length; i++)
			controller.position.coords[i] = this.coord("position." + i, world.getSize().coords[i] + (i == 1 ? -2 : (i == 3 ? -0.5 : -1)));
		...
		for (int i = 0; i < controller.vision.coords.length; i++)
			controller.vision.coords[i] = this.coord("vision." + i, i == 3 ? 0.5 : world.getSize().coords[i]);
		...
		if (controller instanceof UserController) {
			...
			controller.placeBlocksOnlyOnFaces = this.bool("placeBlocksOnlyOnFaces");
		}

		controller.inventory = new int[this.get("idInHand", "").split(",").length];
		for (int i = 0; i < controller.inventory.length; i++)
			controller.inventory[i] = (int) toNum(this.get("idInHand", "").split(",")[i]);
		...
		if (controller instanceof DamageableUserController) {
			...
			for (int i = 0; i < controller.spawnPosition.coords.length; i++)
				controller.spawnPosition.coords[i] = this.coord("position." + i, world.getSize().coords[i] + (i == 1 ? -2 : (i == 3 ? -0.5 : -1)));
		}
	}
}